      ...
      <div className="w-full h-full flex flex-col gap-[80px]">
        <div className="w-full flex justify-between items-center mobile:flex-col mobile:gap-4">
          <div className="space-y-6 mobile:space-y-2">
            <h3 className="text-left text-4xl mobile:text-2xl nanum-extra-bold text-neutral-800 mobile:text-center">
              사주 공유하기
            </h3>
            <p className="text-xl font-bold text-neutral-800 mobile:text-sm">
              채팅으로 사주를 공유해보세요
            </p>
          </div>
          <a href="/chat">
...
const ShareCard = ({ title, description, img }) => {
  return (
    <div className="flex flex-col rounded-xl shadow-md max-w-[450px] max-h-[378px] overflow-hidden">
      ...
      <div className="p-5 flex flex-col items-start gap-1.5 mobile:flex-col">
        <h4 className="text-base font-normal text-neutral-800 mobile:text-ml">
          {title}
        </h4>
        <p className="text-xl font-extrabold text-neutral-800 mobile:text-sm">
          {description}
        </p>
      </div>
    </div>
